...
> 最后更新：2026-09-18 · DB v23 · 路径 `D:\MY_Project\to-do_list`
...
|------|------|
| **日期** | 2026-09-18 |
| **做了什么** | 修复 Android Release 缺少自定义音频导致 `invalid_sound`、全部任务提醒无法排程的问题；全屏通知改用新 v3 通道和系统默认声音；首次安装默认页同步为本周；发布版本升级到 `1.3.1+7`。 |
| **下一步** | 构建并发布 `1.3.1+7` Android Release 与更新清单，真机验收应用内升级、锁屏/亮屏、准点、巡检和稍后提醒。 |
| **踩坑** | Flutter/Gradle 中间资源包含 WAV 不代表最终 APK 包含；通知通道引用动态 raw 资源时，最终 Release 资源检查和失败降级不可省略。 |
| **已知问题** | 提醒专项 7/7、完整回归 89/89 通过；旧 `1.3.0+6` APK 仍是坏包。自定义铃声暂缓，`1.3.1+7` APK 与真机尚待发布验收。 |
| **自我改进** | 通知可靠性优先于定制声音；外部资源异常不得让提醒排程整体失败。 |
...
| # | 内容 |
|---|------|
| — | 普通 PowerShell 构建本次全屏提醒 Android Release；Android 14+ 开启“全屏提醒”特殊权限，并完成锁屏/亮屏/权限拒绝降级三组验收 |
| — | 普通 PowerShell/Android Studio 构建 1.3.1+7 正式签名 APK，核对包名、版本、证书和生成时间 |
...
| 批次 | 内容 |
|------|------|
| 9/18 | Android 全屏通知从缺失的自定义声音恢复到系统默认声音和新 v3 通道；见 `docs/2026-09-18-Android全屏通知恢复.md` |
| 9/17 | “稍后 10 分钟”持久化、提醒变更自动失效与 Stride 内置铃声；见 `docs/2026-09-17-稍后提醒持久化与铃声.md` |
| 9/17 | Android 全屏任务提醒、今日任务巡检、提醒设置与双端 `Stride` 更名；见 `docs/2026-09-17-Android全屏提醒与Stride更名.md` |
| 9/17 | Android 与 Windows 六序镜花图标同步、产品显示名统一为 `Stride`；见 `docs/2026-09-17-双端应用图标同步.md` |
| 9/17 | Android 自动更新检查、手动反馈、15 秒总超时与非强制稍后；见 `docs/2026-09-17-Android自动更新检查反馈修复.md` |
| 9/17 | 本周短日期、今天首屏锚点、其他周周首与空今天快捷添加；见 `docs/2026-09-17-本周短日期与今日首屏定位.md` |
