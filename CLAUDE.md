...
|------|------|
| **日期** | 2026-09-17 |
| **做了什么** | 本周日期标题压缩为“周四 · 9/17”；当前周改用今天日期卡作为滚动锚点，其他周从周首开始，今天为空时仍默认显示快捷添加。 |
| **下一步** | 小花先生在普通 PowerShell/Android Studio 构建 Android Release，真机检查首屏今日定位、向上查看过去日期和详情返回位置。 |
| **踩坑** | 自动滚动不应依赖尚未构建的列表子项；使用 `CustomScrollView.center` 直接建立今天锚点，比首帧后猜测像素偏移稳定。 |
| **已知问题** | 提升权限后 Flutter 测试和 Windows 构建已完成；Android Gradle 仍在 Codex/JBR 环境报 `Unable to establish loopback connection`。 |
| **自我改进** | 当前周、其他周和详情返回是三种不同滚动语义：今天锚点、周首、保留位置，测试必须分别覆盖。 |
