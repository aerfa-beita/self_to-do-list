...
|------|------|
| **日期** | 2026-09-16 |
| **做了什么** | 小花先生确认安排页 B 方案；已实现“安排”双层顶部、三列紧凑摘要、单行周导航、状态筛选、日期/状态星期卡片和圆形完成控件，并更新设计规范与视觉基准。 |
| **下一步** | 进入应用内更新阶段：采用 GitHub Releases 承载 APK 与版本清单，先建立正式签名和发布脚本，再实现检查、下载、校验与系统安装确认。 |
| **踩坑** | 桌面二级菜单不能在一级菜单反向动画未结束时立即打开；整卡 `BuildContext` 可能覆盖整屏，必须延迟并校验锚点范围。Flutter 批处理脚本会卡在 SDK 包装层，直接调用 SDK snapshot 可正常测试。 |
| **已知问题** | Android Release 仍有既有 Codex/JBR `Unable to establish loopback connection`，需普通 PowerShell 或 Android Studio 构建；当前无 Android 设备或 AVD，真机状态栏、底部手势区和长按手感尚未验收。 |
| **自我改进** | 视觉基准要从独立页面状态启动，避免跨视图截图继承滚动位置；摘要、导航、状态筛选分别承担统计、时间范围和过滤职责。 |
