...
|------|------|
| **日期** | 2026-09-16 |
| **做了什么** | 完成 GitHub Releases 应用内更新：24 小时/手动检查、进度与取消、SHA-256/包名/版本/签名校验、未知来源授权、系统安装确认、正式签名和发布脚本；版本升至 1.3.0+6。 |
| **下一步** | 在普通 PowerShell/Android Studio 构建正式签名 Android Release，完成首次“导出数据—卸载调试版—安装正式版—导入”迁移；再用更高 versionCode 的测试 Release 验收覆盖升级后公开发布。 |
| **踩坑** | Flutter 依赖解析会顺带刷新允许范围内的间接依赖；提交前需把 `pubspec.lock` 收窄为本批必需变化。Codex/JBR 仍在 Gradle 启动阶段报回环连接错误。 |
| **已知问题** | Android 原生 Kotlin、正式 APK、未知来源授权与真实覆盖安装尚未通过构建/真机验收；GitHub Release 未发布；签名密钥目前只有同机恢复副本，仍需异地加密备份。 |
| **自我改进** | 更新链应同时校验传输摘要与应用签名；发布顺序必须 APK 在前、版本清单在后；Release 缺密钥时应直接失败。 |
