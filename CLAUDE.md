...
|------|------|
| **日期** | 2026-09-18 |
| **做了什么** | 修复 Android Release 缺少自定义音频导致 `invalid_sound`、全部任务提醒无法排程的问题；全屏通知改用新 v3 通道和系统默认声音；首次安装默认页同步为本周；`1.3.1+7` APK 与更新清单已发布。 |
| **下一步** | 真机验收应用内从 `1.3.0+6` 升级到 `1.3.1+7`，以及锁屏/亮屏、准点、巡检和稍后提醒。 |
| **踩坑** | Flutter/Gradle 中间资源包含 WAV 不代表最终 APK 包含；通知通道引用动态 raw 资源时，最终 Release 资源检查和失败降级不可省略。 |
| **已知问题** | 提醒专项 7/7、完整回归 89/89 通过；旧 `1.3.0+6` APK 仍是坏包。自定义铃声暂缓，`1.3.1+7` 已发布但仍待真机验收。 |
...
| # | 内容 |
|---|------|
| — | Android 14+ 开启“全屏提醒”特殊权限，并完成锁屏/亮屏/权限拒绝降级三组验收 |
