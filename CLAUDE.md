...
|------|------|
| **日期** | 2026-09-17 |
| **做了什么** | Android 自动更新检查改为成功后 24 小时节流；手动检查不再被后台请求吞掉，增加 15 秒总超时和分类反馈；所有更新允许稍后。 |
| **下一步** | 发布首个包含 APK 与 `update-manifest.json` 的 GitHub Release，真机验收自动/手动检查、断网、下载、授权与覆盖安装。 |
| **踩坑** | 连接超时不覆盖响应正文；更新时间在请求前写入会把失败误当成功；自动与手动请求共用状态时必须保留手动反馈意图。 |
| **已知问题** | Flutter 82/82 和 Windows Release 已完成；Android Gradle 仍在 Codex/JBR 环境报 `Unable to establish loopback connection`。 |
| **自我改进** | 自动任务保持安静不等于手动操作可以无反馈；共享请求必须按用户发起时刻升级反馈级别。 |
