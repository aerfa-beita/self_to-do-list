...
|------|------|
| **日期** | 2026-09-16 |
| **做了什么** | 建立 `963e0b3` 安排优先检查点；完成本周历史接线修复；已完成按 `completed_at`、最近删除按 `deleted_at` 从新到旧排序，来源隔离不变。 |
| **下一步** | 在普通 PowerShell 补跑 `flutter test --concurrency=1`；随后实现最近删除专用卡片，再进入安排页视觉方案。 |
