...
|------|------|
| **日期** | 2026-09-17 |
| **做了什么** | Android 任务到点与今日巡检使用带 Stride 铃声的全屏提醒；“稍后 10 分钟”按任务 syncId 持久化，重启/同步后保留，完成、删除、修改提醒或到期后清理；Android/Windows 产品名统一为 `Stride`。 |
| **下一步** | 在普通 PowerShell 构建最新 Android Release，真机分别验收锁屏/亮屏、Android 14 全屏特殊权限、准点与巡检重合、完成和稍后提醒。 |
| **踩坑** | Android 通知通道的铃声创建后不可修改，因此新增 `stride_full_screen_reminders_with_sound_v2` 通道；延后提醒不能只保存在系统闹钟里，否则启动时重排会覆盖。 |
| **已知问题** | Flutter 88/88、3 张实际组件基准、铃声 `aapt2` 编译与 Windows Release 已通过；20:58 Android 单次重试仍报 Gradle/JBR 回环故障，19:49 的旧 APK 禁止作为验收包。 |
| **自我改进** | 全屏提醒必须区分“应用内设计已完成”“系统允许全屏”和“真机锁屏已验收”；没有新 APK 与真机证据时不能宣称完整通过。 |
