...
|------|------|
| **日期** | 2026-09-16 |
| **做了什么** | DB 升至 v23 并创建 `.pre-v23` 备份；移除精灵窝入口和自动暂存；旧暂存任务按原阶段/日期释放；主导航改为“安排｜备忘录”，页面记忆改用语义值并兼容旧数字设置。 |
| **下一步** | 让小花先生在安排页方案 A/B 中确认一个方向；确认后更新 `DESIGN.md` 并实现安排页视觉改版，然后再进入应用内更新。 |
| **踩坑** | 桌面二级菜单不能在一级菜单反向动画未结束时立即打开；整卡 `BuildContext` 可能覆盖整屏，必须延迟并校验锚点范围。Flutter 批处理脚本会卡在 SDK 包装层，直接调用 SDK snapshot 可正常测试。 |
| **已知问题** | Android Release 仍有既有 Codex/JBR 回环故障，需普通 PowerShell 或 Android Studio 构建；当前无 Android 设备或 AVD。两张安排页方案图未确认，`DESIGN.md` 尚未更新。 |
| **自我改进** | 导航记忆应存语义值而非数字下标；停用功能时先解除业务分流并保留兼容字段，等旧客户端退出后再做破坏性删列。 |
