...
class MemoRepository {
  ...
  Future<int> insertChild(Memo memo, int parentId) async {
    final maxOrder = await _db.rawQuery(
        'SELECT COALESCE(MAX(sort_order), -1) as m FROM memos WHERE parent_id = ?', [parentId]);
    final nextOrder = ((maxOrder.first['m'] as int?) ?? -1) + 1;
    final map = memo.toMap()..remove('id');
  ...
  Future<int> update(Memo memo) async {
    return await _db.update('memos', memo.toMap(), where: 'id = ?', whereArgs: [memo.id]);
  }

  Future<void> _softDeleteSubTree(int id) async {
    final children = await _db.query('memos',
        where: 'parent_id = ? AND deleted_at IS NULL', whereArgs: [id]);
    for (final child in children) {
      await _softDeleteSubTree(child['id'] as int);
    }
    await _db.update('memos', {'deleted_at': DateTime.now().toIso8601String()},
  ...
  Future<void> softDelete(int id) async {
    ...
    // 移到同层末尾
    if (self.isNotEmpty) {
      final whereClause = parentId != null ? 'parent_id = ? AND deleted_at IS NULL' : 'parent_id IS NULL AND deleted_at IS NULL';
      ...
      await _db.update('memos', {'sort_order': lastOrder}, where: 'id = ?', whereArgs: [id]);
    }
  }

  Future<void> restore(int id) async {
    await _db.update('memos', {'deleted_at': null}, where: 'id = ?', whereArgs: [id]);
  }

  Future<void> permanentlyDelete(int id) async {
    final children = await _db.query('memos', where: 'parent_id = ?', whereArgs: [id]);
    for (final child in children) {
  ...
  Future<void> cleanupOldDeleted() async {
    ...
    final old = await _db.query('memos',
  ...
  Future<List<Memo>> getRoots() async {
    ...
    final maps = await _db.query('memos',
  ...
  Future<List<int?>> promote(int id) async {
    ...
    final parent = await _db.query('memos', where: 'id = ?', whereArgs: [oldParent]);
  ...
  Future<List<int?>> demote(int id) async {
    ...
    final int? oldParent = st['parent_id'] as int?;
    final siblings = await _db.query('memos',
        where: oldParent == null ? 'parent_id IS NULL AND deleted_at IS NULL' : 'parent_id = ? AND deleted_at IS NULL',
    ...
    final children = await _db.query('memos', where: 'parent_id = ?', whereArgs: [newParent]);
  ...
  Future<void> moveUp(int id) async {
    ...
    final whereClause = parentId != null ? 'parent_id = ? AND deleted_at IS NULL' : 'parent_id IS NULL AND deleted_at IS NULL';
    final whereArg = parentId;
    final above = await _db.query('memos',
    ...
    await _db.update('memos', {'sort_order': aboveOrder}, where: 'id = ?', whereArgs: [id]);
    await _db.update('memos', {'sort_order': myOrder}, where: 'id = ?', whereArgs: [aboveId]);
  }
  ...
  Future<void> moveDown(int id) async {
    ...
    final whereClause = parentId != null ? 'parent_id = ? AND deleted_at IS NULL' : 'parent_id IS NULL AND deleted_at IS NULL';
    final whereArg = parentId;
    final below = await _db.query('memos',
        where: '$whereClause AND sort_order > ?',
        whereArgs: whereArg != null ? [whereArg, myOrder] : [myOrder],
        orderBy: 'sort_order ASC', limit: 1);
    if (below.isEmpty) return;
    final belowOrder = below.first['sort_order'] as int;
    final belowId = below.first['id'] as int;
    await _db.update('memos', {'sort_order': belowOrder}, where: 'id = ?', whereArgs: [id]);
    await _db.update('memos', {'sort_order': myOrder}, where: 'id = ?', whereArgs: [belowId]);
  }
}
